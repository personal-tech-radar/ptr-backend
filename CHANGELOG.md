...
- Validated saved-article and feedback IDs before database access and made first-open, save/unsave,
  and feedback aggregate writes atomic inside PostgreSQL transactions.
- Sanitized digest BullMQ identities while preserving logical period keys for real scheduler runs.
- Added the idempotent legacy-user and immutable source-manifest production bootstrap migration.
- Added concise module documentation and documented the external `APP_URL` contract.

- Stabilized taxonomy source discovery so one logical request survives BullMQ retries with explicit
  queued/running/completed/failed state and attempt metadata, retained deterministic jobs are safely
  replaced for truthful administrator retries, and provider failures are sanitized before BullMQ
  persists failed history.

- Made article-analysis retries recoverable across retained BullMQ history, added deterministic
  dated-before-undated public ordering, aligned candidate retry on HTTP 200, and centralized
  manifest/candidate source creation behind advisory-locked identity resolution.
- Preserved valid RSS/Atom and extracted website publication dates while defining malformed or
  absent dates as nullable with deterministic public-feed fallback ordering.
- Detect submitted URLs that are already RSS/Atom feeds before website discovery, and sanitize
  provider authentication failures so API-key fragments never reach structured logs.
- Preserved HTTP 413 for oversized JSON with a stable `PAYLOAD_TOO_LARGE` error code, enforced
  real-calendar `beforeDate` validation, and added `DISCOVERY_LIMIT_REACHED` to quota responses.
- Made identical onboarding submissions true cache/persistence no-ops by comparing timezone,
  experience level, taxonomy, and stream sets before synchronization and invalidation.
- Fixed profile settings so daily/weekly digest opt-ins are editable, display names cannot be
  blank, and GitHub profile URLs must use HTTPS on github.com.
- Made onboarding stream selection replacement transactional and made profile cache invalidation
  conditional on actual feed-affecting changes.
- Allowed authenticated users to complete onboarding before email verification while retaining
  the combined verification-and-onboarding gate for personal feeds and scheduled digests.
- Added distinct `EMAIL_NOT_VERIFIED` personal-feed rejection, nullable pre-onboarding timezones,
  a minimal reusable interest catalog, and a retrievable local mail-capture API.
- Aligned normal-user login, refresh, and logout success responses with their documented HTTP 200
  status codes.
...
## [dev/mvp3-admin-security] — 2026-08-02

### Added

- Separate administrator persistence, short-lived administrator JWT authentication, token-version
  password invalidation, presented-token logout, administrator creation/listing, and idempotent bootstrap
- Public article DTOs with taxonomy, streams, summaries, safe analysis metadata, and separate public
  click/personal tracked-open counters
- Dedicated personal and public redirect routes, administrator preview digest metadata, and ignored
  per-iteration implementation reports

### Updated

- All dashboard routes now live under `/admin/**` and accept only administrator JWTs; public content
  reads accept only `X-API-KEY`; personal operations accept only user JWTs
- `GET /public/feed` is API-key protected while anonymous preview remains credential-free
- Feedback is create/replace/idempotent only; user deletion of explicit feedback was removed
- Swagger now declares separate user, administrator, API-key, and opaque public access boundaries
- Swagger operations now include behavior-focused descriptions, explicit administrator/source
  semantics, concrete source coverage and user-source-interaction response schemas, and a
  public-to-user-to-administrator tag order
- Digest emails and administrator digest responses now expose the same permanent temporary
  digest-stream page links for browser preview by stream

### Removed

- Administrator roles and administrator records from the users domain, hybrid API-key/JWT admin
  authorization, legacy unprefixed dashboard routes, and feedback deletion
- Deprecated `/go/:uuid` personal redirects and signed `/articles/:articleId/save-from-email`
  links; personal links now use `/r/:uuid`, JWT saves use `/saved-articles/:articleId`, and digest
  email saves use permanent opaque `/email-action/:id` links

## [dev/mvp3-final] — 2026-07-31

### Added

- Final integrated MVP3 source discovery, deterministic candidate lifecycle, durable discovery quota, provenance coverage, source health, and operational administration
- One five-minute scheduler orchestrator, stream-frequency ingestion priorities, stream-versioned feed caches, internal exporter-neutral metrics, and 30-day technical-history cleanup
- Permanent digest feedback/save actions, private digest-stream pages, immutable statistics snapshots, and idempotent local-period digest delivery

### Updated

- Registration/onboarding, global pre-analysis, deterministic source-based personalization, daily/weekly digest selection, email rendering, and admin queue operations now follow the final cumulative MVP3 specification
- Daily digest delivery is every day at 09:00 local; weekly delivery remains Friday at 14:00 local
- Article difficulty is normalized to beginner/intermediate/advanced, with historical architect values migrated to advanced

### Removed

- Manual-review candidate states, source discovery from article links, competing source/digest cron orchestration, direct article-feedback scoring, and inline digest ingestion/analysis

### Compatibility

- The unified `TechnologyInterest` model, browser-fetch isolation queue, existing source extraction/retry components, admin bootstrap, legacy-user migration, and database-backed multi-user foundation are retained
...
- Personal Digest Delivery (MVP3 Part 1, Phase 10 of 11) — daily and weekly digest emails are built per user. The scheduler checks daily delivery at 09:00 every calendar day and weekly delivery Friday at 14:00 in each user's timezone.
