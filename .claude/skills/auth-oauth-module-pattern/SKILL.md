...
- Read the signing secret through a dedicated helper (e.g. `getJwtSecret()`) that throws if the env var is unset — never fall back to a hardcoded literal. This is the general "fail fast on a missing secret" rule from `CLAUDE.md`, applied to JWT signing specifically.
